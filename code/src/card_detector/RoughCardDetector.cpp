...
void RoughCardDetector::loadPreset(const PipelinePreset preset) {
  ...
  case PipelinePreset::SINGLE_CARD:
    add_step(preprocessing::hsvWhiteThreshold, cv::Scalar(0,0,200), cv::Scalar(179,20,255), 1.2, 10.0);
...
cv::Mat keepLargestObject(const cv::Mat &maskIn) {
  ...
  cv::findContours(maskIn, contours, cv::RETR_EXTERNAL, cv::CHAIN_APPROX_SIMPLE);

  if (contours.empty()) {
    return cv::Mat::zeros(maskIn.size(), CV_8UC1);
  }
  ...
  cv::Mat out = cv::Mat::zeros(maskIn.size(), CV_8UC1);
  cv::drawContours(out, contours, static_cast<int>(maxIdx), cv::Scalar(255), cv::FILLED);
  return out;
}
